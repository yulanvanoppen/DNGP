function plot_filtered(model, modelA, modelB, E, F)
    figure(Position=[100 100 950 350])
    tl = tiledlayout(1, 4, TileSpacing='compact');
    tl.XLabel.String = "{\itt} (cell cycle progression)";
    tl.YLabel.String = "\itY";
    
    plot_data(modelA, modelB)
    plot_fit(model, modelA, modelB)
    plot_mpost(model, modelA, modelB)

    plot_indicators(model, modelA, modelB, E, F)
end


function plot_data(modelA, modelB)
    t = modelA.t;
    data1 = modelA.y;
    data2 = modelB.y;
    R1 = modelA.R;
...
function plot_mpost(model, modelA, modelB)
    t = model.t;
    c = 2;
    ...
    h2 = ax.Children(5);
    h3 = ax.Children(2);
    legend([h1, h2, h3], 'Data', 'Residual GP', 'Mean GP posterior', location='north')
...
function plot_indicators(model, modelA, modelB, E, F)
    ...
    for n = 1:length(t)
        ...
        if n == 1, xl = 0; end
        if n == length(t), xr = model.t(end); end
    ...
    plot(t, [mu1, mu1 + c*s1, mu1 - c*s1], ':', 'LineWidth', 3, 'Color', [0 0.4470 0.7410 .5])
    plot(t, [mu2, mu2 + c*s2, mu2 - c*s2], ':', 'LineWidth', 3, 'Color', [0.8500 0.3250 0.0980 .5])
    plot(t, [mu, mu + c*s, mu - c*s], ':', 'LineWidth', 3, 'Color', [0.9290 0.6940 0.1250 .5])
    % plot(t, [mu + s, mu - s], ':', 'LineWidth', 2, 'Color', [0.9290 0.6940 0.1250 .6])
    
    xlim([0 model.t(end)])
    xticks(model.t(end) .* (0:.25:1))
    xticklabels(["0" "0.25" "0.5" "0.75" "1"])
    % xticks([1 xticks])
    ylim(lim)
    % xlabel('\itt')
